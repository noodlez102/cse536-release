...
usertrapret(void)
{
  ...
  mythread()->trapframe->kernel_hartid = r_tp();         // hartid for cpuid()
  ...
  printf("sepc is %p, sstatus is %d, satp is %p, and sp is %p\n", r_sepc(), r_sstatus(), r_satp(), r_sp());
...
kerneltrap()
{
  ...
  // give up the CPU if this is a timer interrupt.
  if(which_dev == 2 && myproc() != 0 && mycpu()->tid != -1 && mythread()->state == RUNNING)
      yield();
